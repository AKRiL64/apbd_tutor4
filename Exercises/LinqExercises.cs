...
public sealed class LinqExercises
{
    ...
    {
        var query = UniversityData.Students.OrderBy(s => s.FirstName)
            .ThenBy(s => s.LastName)
    ...
    {
        //TODO
        var query = UniversityData.Courses.FirstOrDefault(c => c.Category == "Analytics");
        if (query != null)
        {
            //return query.;
        }
